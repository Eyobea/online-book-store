  ...
  if (empty($name) || empty($email) || empty($number) || empty($address) || empty($city) || empty($state) || empty($country) || empty($pincode) || empty($account_paid) || empty($cashier_account)) {
    ...
    $order_query = mysqli_query($conn, "SELECT * FROM confirm_order WHERE name = '$name' AND number = '$number' AND email = '$email' AND payment_method = '$method' AND address = '$address' AND total_books = '$total_books' AND total_price = '$cart_total'") or die('query failed');
      ...
      if (mysqli_num_rows($order_query) > 0) {
        ...
        mysqli_query($conn, "INSERT INTO confirm_order(user_id, name, number, email,account_paid, address,total_books, total_price,account_paid_status,total_price_status, order_date) VALUES('$user_id','$name', '$number','$account_paid','$account_paid_status','$total_price_status', '$email','$method', '$full_address', '$total_books', '$cart_total', '$placed_on')") or die('query failed');
  
        $conn_oid= $conn->insert_id;
        $_SESSION['id'] = $conn_oid;
        // $select_book = mysqli_query($
          ...
          <div class="row">
            <div class="col-50">
              ...
              <div style="padding: 0px;" class="row">
                ...
                <div class="col-50">
                  <label for="paid">Account Paid</label>
                  <input type="text" id="paid" name="account_paid" placeholder="">
                </div>
                <div class="col-50">
                  <label for="account">Manager Account</label>
                  <input type="text" id="account" name="cashier_account" placeholder="">
                </div>
                <div class="col-50">
