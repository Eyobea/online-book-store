...
if (isset($_POST['login'])) {
    ...
    if (mysqli_num_rows($select_users) == 1) {
        ...
        } elseif ($row['user_type'] == 'Accountant') { // Adding functionality for accountants
            $_SESSION['accountant_name'] = $row['name'];
            $_SESSION['accountant_email'] = $row['email'];
            $_SESSION['accountant_id'] = $row['Id'];
            header('location: accountant.php'); // Change the redirection to the appropriate page
        } elseif ($row['user_type'] == 'Cashier') { 
            $_SESSION['cashier_name'] = $row['name'];
            $_SESSION['cashier_email'] = $row['email'];
            $_SESSION['cashier_id'] = $row['Id'];
            header('location: cashier.php'); // Change the redirection to the appropriate page
        }
    } else {
...
<body>
    ...
    <div class="container">
        <form action="" method="post">
            <h3 style="color:white">login to <a href="index.php"><span>ONLINE BOOK STORE </span></a></h3>
