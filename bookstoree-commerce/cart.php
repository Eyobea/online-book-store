...
if (isset($_GET['remove'])) {
    ...
    // Redirect to avoid resubmission if the user refreshes the page
...
if (isset($_POST['update'])) {
    ...
    $current_date = date('Y-m-d H:i:s'); // Get current date and time in MySQL format
    // Use prepared statements to prevent SQL injection
    $stmt = $conn->prepare("UPDATE cart SET quantity=?, total=?, date=? WHERE id=?");
    $stmt->bind_param("idsi", $update_quantity, $total_price, $current_date, $update_cart_id);
    $stmt->execute();
    $stmt->close();
...
while ($row = $select_book->fetch_assoc()) {
                ?>
                        <tr id="row_<?php echo $row['id']; ?>"> <!-- Add id to the row -->
                ...
                <tr>
                    ...
                    <th colspan="2"> birr <?php echo $total; ?></th>
                </tr>
